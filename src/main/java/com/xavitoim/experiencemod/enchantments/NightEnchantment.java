...
public class NightEnchantment extends Enchantment {
    ...
    public NightEnchantment(Rarity rarityIn, EnchantmentType typeIn, EquipmentSlotType[] slots) {
    ...
    public static class NightEvent {
        ...
        public static void nightEvent(TickEvent.PlayerTickEvent event) {
            if(!event.player.getItemStackFromSlot(EquipmentSlotType.HEAD).isEmpty()){
                ...
                if (enchantmentsOnItemOnHead.containsKey(EnchantmentInit.NIGHT.get())) {
                    NightEventLogic.nightEvent(event);
                }
            }
        }
    }
